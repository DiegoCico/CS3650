...
#include <string.h>
#include <ctype.h>
#include "tokenizer.h"  // Use the header we just created

// Tokenizer function: Given an input string, returns an array of tokens.
...
char **tokenize(const char *input, int *numTokens) {
        ...
        switch (state) {
            ...
            case STATE_IN_WORD:
                if (isspace(c) || c == '(' || c == ')' || c == '<' ||
                    ...
                    tokens[tokensSize++] = token;
                    state = STATE_NONE;
                    // Do not increment i here; the special character is processed next.
                } else {
                    i++;
                }
                break;
    ...
    // If we ended while still in a word, flush the token.
